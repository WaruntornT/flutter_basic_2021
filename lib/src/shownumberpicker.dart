...
class ShowNumberPicker extends StatefulWidget {
  const ShowNumberPicker({Key key}) : super(key: key);

  @override
  _ShowNumberPickertate createState() => _ShowNumberPickertate();
}

class _ShowNumberPickertate extends State<ShowNumberPicker> {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            SingleChildScrollView(child: _IntegerExample()),
            SingleChildScrollView(child: _DecimalExample()),
          ],
        ),
      ),
    );
    ...
    return Column(
      children: <Widget>[
        ...
        NumberPicker(
          ...
          minValue: 0, //เริ่มต้น
          maxValue: 100, //สิ้นสุด
          step: 10, // จำนวนการนับ
          haptics: true,
          onChanged: (value) { //(value) => setState(() => _currentIntValue = value),
            setState(() { _currentIntValue = value; });
            print("_currentIntValue : ${_currentIntValue}");
          },
        ),
        SizedBox(height: 32),
        Row(
